...
export async function importRoutes(routesDirectory: string, routePathPrefix = ''): Promise<Routes> {
    const result: Routes = {};

    const promises = readdir(routesDirectory).map(async (file) => {
        if (file.isDirectory) {
            const routes = await importRoutes(file.absPath, `${routePathPrefix}/${file.basename}`);
            Object.assign(result, routes);
        } else if (file.basename.endsWith('.js')) {
            const routePath =
                file.basename === 'index.js'
                    ? routePathPrefix
                    : `${routePathPrefix}/${file.basename.substring(0, file.basename.length - 3)}`;

            const modules = (await import(file.absPath)) as Record<string, Route>;
            ...
            Object.entries(modules).forEach(([name, module]) => {
                ...
                if (!isValidMethod(method)) {
                    ...
                    console.warn(`Ignoring invalid method "${method}" in ${file.absPath}`);
                    return;
                }
